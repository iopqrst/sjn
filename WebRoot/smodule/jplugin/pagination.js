	...
	(function($) {
		...
		$.extend($.PaginationCalculator.prototype, {
			...
			getInterval : function(current_page) {
				...
				var end = current_page > ne_half ? Math.min(current_page
						+ ne_half + (this.opts.num_display_entries % 2), np)
		...
		$
				.extend(
						...
						{
							/**
							 * Helper function for generating a single link (or
							 * a span tag if it's the current page)
							 * 
							 * @param {Number}
							 ...
							 *            current_page
							 * @param {Object}
							 *            appendopts Options for the new item:
							 *            text and classes
							 * @returns {jQuery} jQuery object containing the
							 *          link
							 */
							createLink : function(page_id, current_page,
								...
								page_id = page_id < 0 ? 0
										: (page_id < np ? page_id : np - 1); // Normalize
																				// page
																				// id
																				// to
																				// sane
							...
							appendRange : function(container, current_page,
									start, end, opts) {
								var i;
								for (i = start; i < end; i++) {
							...
							getLinks : function(current_page, eventHandler) {
								...
								// Fixed 2013-05-25 HZQ
								// var $jumpFn = $("<span
								// class=\"tip\">跳转到</span>" +
								// "<input id='pageInput' name='pageInput'
								// value='" + (current_page + 1) + "'/>" +
								// "<a id=\"jump\" href='javascript:void(0);'
								// title=\"跳转\">跳转</a>");
								//				
								// fragment.append($jumpFn);
		...
		$.fn.pagination = function(maxentries, opts) {
			...
			/**
			 * This is a utility function for the internal event handlers. It
			 * sets the new current page on the pagination container objects,
			...
			function selectPage(new_current_page) {
				...
				links = renderer.getLinks(new_current_page,
			...
			containers
					.bind(
							...
							function(evt, page_id) {
								...
								page_id = (page_id + 1) <= 0 ? 0 : page_id;
								page_id = (page_id + 1) > evt.data.numPages ? evt.data.numPages - 1
										: page_id;
