define(function(require, exports, module) {
	 ...
	 * 
	 * Copyright 2013-2014 bootstrap-select Licensed under MIT
	...
	(function($) {
		...
		var Selectpicker = function(element, options, e) {
			...
			// If we have no title yet, check the attribute 'title' (this is
			// missed by jq as its not a data-attribute
		...
		// part of this is duplicated in i18n/defaults-en_US.js. Make sure to
		// update both.
		...
		Selectpicker.prototype = {
			...
			createLi : function() {
				var that = this, _liA = [], _liHtml = '', optID = 0;
				...
				this.$element
						...
						.each(function() {
							...
							// Get the class and text for the option
								...
								var subtext = $this.data('subtext') !== undefined ? '<small class="muted text-muted">' + $this
										.data('subtext') + '</small>'
										: '';
								...
								if (that.options.hideDisabled
									...
									if ($this.index() === 0) {
										...
										var labelSubtext = $this.parent().data(
												'subtext') !== undefined ? '<small class="muted text-muted">' + $this
										...
										_liA.push(that.createA(text,
												'opt ' + optionClass, inline,
												optID));
									}
				...
				$.each(_liA, function(i, item) {
					var hide = item === '<a></a>' ? 'class="hide is-hidden"'
							: '';
					_liHtml += '<li rel="' + i + '"' + hide + '>' + item
							+ '</li>';
				});

				// If we are not multiple, and we dont have a selected item, and
				// we dont have a title, select the first element so something
			...
			createA : function(text, classes, inline, optgroup) {
				return '<a tabindex="0" class="'
						...
						+ (typeof optgroup !== 'undefined' ? 'data-optgroup="' + optgroup + '"'
								: '') + '>' + text + '<i class="'
						+ this.options.iconBase + ' ' + this.options.tickIcon
			...
			render : function(updateLi) {
						...
						.map(
								function() {
									...
									if (that.options.showSubtext
											...
											&& !that.multiple) {
										subtext = ' <small class="muted text-muted">' + $this
												.data('subtext') + '</small>';
									} else {
										subtext = '';
									}
				...
				// Fixes issue in IE10 occurring when no default option is
				// selected and at least one option is disabled
				// Convert all the values into a comma delimited string
				var title = !this.multiple ? selectedItems[0] : selectedItems
						.join(this.options.multipleSeparator);

				// If this is multi select, and the selectText type is count,
				// the show 1 of 2 selected etc..
				if (this.multiple
					...
					if ((max.length > 1 && selectedItems.length > max[1])
						...
						title = this.options.countSelectedText
								...
								.replace(
										'{1}',
										this.$element
												.find('option:not([data-divider="true"], [data-hidden="true"])' + notDisabled).length);
					}
				}
				...
				// If we dont have a title, then use the default, or if nothing
				// is set at all, use the not selected text
				if (!title) {
					title = this.options.title !== undefined ? this.options.title
							: this.options.noneSelectedText;
				}

				this.$button.attr('title', $.trim(
						$("<div/>").html(title).text()).replace(/\s\s+/g, ' '));
				...
				if (this.$element.attr('class')) {
					this.$newElement
							.addClass(this.$element
									.attr('class')
									.replace(
											/selectpicker|mobile-device|validate\[.*\]/gi,
											''));
				}
			...
			liHeight : function() {
				...
				var $selectClone = this.$menu.parent().clone().find(
						...
						: 0, actionsHeight = this.options.actionsBox ? $menuClone
						.find('.bs-actionsbox').outerHeight()
						: 0;

				$selectClone.remove();

				this.$newElement.data('liHeight', liHeight).data(
						'headerHeight', headerHeight).data('searchHeight',
						searchHeight).data('actionsHeight', actionsHeight);
			},

			setSize : function() {
				var that = this, menu = this.$menu, menuInner = menu
						.find('.inner'), selectHeight = this.$newElement
						...
						.data('searchHeight'), actionsHeight = this.$newElement
						.data('actionsHeight'), divHeight = menu.find(
						'li .divider').outerHeight(true), menuPadding = parseInt(menu
						.css('padding-top'))
					...
					// JQuery defines a scrollTop function, but in pure JS it's
					// a property
				...
				if (this.options.size == 'auto') {
					var getSize = function() {
						...
						menu.css( {
							...
							'overflow' : 'hidden',
							'min-height' : minHeight + headerHeight
									+ searchHeight + actionsHeight + 'px'
						});
						menuInner
								.css( {
									'max-height' : menuHeight - headerHeight
											- searchHeight - actionsHeight
											- menuPadding + 'px',
									'overflow-y' : 'auto',
									'min-height' : Math.max(minHeight
											- menuPadding, 0) + 'px'
								});
					};
					getSize();
					...
					$(window).off('resize.getSize').on('resize.getSize',
							getSize);
					$(window).off('scroll.getSize').on('scroll.getSize',
							getSize);
				} else if (this.options.size
						&& this.options.size != 'auto'
						&& menu.find('li' + notDisabled).length > this.options.size) {
					var optIndex = menu.find('li' + notDisabled + ' > *').not(
							'.div-contain').slice(0, this.options.size).last()
							.parent().index();
					var divLength = menu.find('li').slice(0, optIndex + 1)
			...
			setWidth : function() {
				if (this.options.width == 'auto') {
					...
					var ulWidth = selectClone.find('> .dropdown-menu').css(
							'width');
					var btnWidth = selectClone.css('width', 'auto').find(
							'> button').css('width');
					selectClone.remove();

					// Set width to whatever's larger, button title or longest
					// option
					this.$newElement.css('width', Math.max(parseInt(ulWidth),
							parseInt(btnWidth)) + 'px');
				} else if (this.options.width == 'fit') {
					// Remove inline min-width so width can be changed from
					// 'auto'
			...
			mobile : function() {
				this.$element.addClass('mobile-device').appendTo(
						this.$newElement);
				if (this.options.container)
					this.$menu.hide();
			},
			...
			setDisabled : function(index, disabled) {
				if (this.$lis == null)
					this.$lis = this.$menu.find('li');
				if (disabled) {
					$(this.$lis[index]).addClass('disabled').find('a').attr(
			...
			tabIndex : function() {
				if (this.$element.is('[tabindex]')) {
					this.$element.data('tabindex', this.$element
							.attr('tabindex'));
					this.$button.attr('tabindex', this.$element
							.data('tabindex'));
				}
			},
						...
						.on(
								...
								function(e) {
									...
									if (!that.isDisabled()
										...
										// Else toggle the one we have chosen if
										// we are multi select.
										else {
											$option.prop('selected', !state);
											that.setSelected(clickedIndex,
													!state);
											$(this).blur();

											if ((maxOptions !== false)
													|| (maxOptionsGrp !== false)) {
												var maxReached = maxOptions < $options
														.filter(':selected').length, maxReachedGrp = maxOptionsGrp < $optgroup
														.find('option:selected').length, maxOptionsArr = that.options.maxOptionsText, maxTxt = maxOptionsArr[0]
														.replace('{n}',
																maxOptions), maxTxtGrp = maxOptionsArr[1]
														.replace('{n}',
													...
													if (maxOptions
														...
														if (maxOptionsArr[2]) {
															maxTxt = maxTxt
																	.replace(
																			'{var}',
																			maxOptionsArr[2][maxOptions > 1 ? 0
																					: 1]);
															maxTxtGrp = maxTxtGrp
																	.replace(
																			'{var}',
																			maxOptionsArr[2][maxOptionsGrp > 1 ? 0
																					: 1]);
														}
			...
			liveSearchListener : function() {
				...
				this.$searchbox.on('input propertychange', function() {
					if (that.$searchbox.val()) {
						that.$lis.not('.is-hidden').removeClass('hide').find(
								'a').not(
								':icontains(' + that.$searchbox.val() + ')')
			...
			keydown : function(e) {
				...
				if (/(38|40)/.test(e.keyCode.toString(10))) {
					...
					first = $items.parent(':not(.disabled):visible').first()
							.index();
					last = $items.parent(':not(.disabled):visible').last()
							.index();
					...
					if (that.options.liveSearch) {
						...
						first = $items.filter(':not(.disabled):visible')
								.first().data('index');
						last = $items.filter(':not(.disabled):visible').last()
								.data('index');
						next = $items.eq(index).nextAll(
								':not(.disabled):visible').eq(0).data('index');
						prev = $items.eq(index).prevAll(
								':not(.disabled):visible').eq(0).data('index');
					...
					if (!that.options.liveSearch) {
						...
						if (!$this.is('.dropdown-toggle')) {
							...
							$items.eq(index).addClass('active').find('a')
									.focus();
							$this.focus();
						}
					}
					...
					$items.each(function() {
						if ($(this).parent().is(':not(.disabled)')) {
							if ($.trim($(this).text().toLowerCase()).substring(
									0, 1) == keyCodeMap[e.keyCode]) {
					...
					prevKey = $.trim($(':focus').text().toLowerCase())
							.substring(0, 1);
				...
				// Select focused option if "Enter", "Spacebar" are pressed
				// inside the menu.
					...
					.each(function() {
						if ($(this).is('select')) {
							var $this = $(this), data = $this
									.data('selectpicker'), options = typeof option == 'object'
									&& option;
							...
							if (typeof option == 'string') {
								// Copy the value of option, as once we shift
								// the arguments
