define(function(require, exports, module) {
	...
	/*
	 * ! jQuery blockUI plugin Version 2.66.0-2013.10.09 Requires jQuery v1.7 or
	 * later
	 * 
	 * Examples at: http://malsup.com/jquery/block/ Copyright (c) 2007-2013 M.
	 * Alsup Dual licensed under the MIT and GPL licenses:
	...
	(function($) {
		...
		$.blockUI.version = 2.66; // 2nd generation blocking at no extra
		// cost!
		...
		$.blockUI.defaults = {
			...
			// style to replace wait cursor before unblocking to correct
			// issue
			...
			// IE issues: 'about:blank' fails on HTTPS and javascript:false
			// is
			// s-l-o-w
			...
			// if true, focus will be placed in the first available input
			// field
			// when
			// page blocking
			focusInput : true,
			...
			// callback method invoked when fadeIn has completed and
			// blocking
			// message is visible
			onBlock : null,
			...
			// callback is
			// passed the element that has been unblocked (which is the
			// window
			// object for page
			...
			// setting this will turn the cursor to a pointer, otherwise
			// cursor
		...
		function install(el, opts) {
			...
			// if an existing element is being used as the blocking content
			// then
			// we capture
			// its current place in the DOM (and current display style) so
			// we
			// can restore
			// it when we unblock
			...
			// blockUI uses 3 layers for blocking, for simplicity they are
			// all
			// used on every platform;
			// layer1 is the iframe layer which is used to supress bleed
			// through
			// of underlying content
			// layer2 is the overlay layer which has opacity and a wait
			// cursor
			// (by default)
			// layer3 is the message content that is displayed while
			// blocking
			...
			lyr2 = $('<div class="blockUI blockOverlay hhhh" style="z-index:' + (z++) + ';display:none;border:none;margin:0;padding:0;width:100%;height:100%;top:0;left:0"></div>');
			...
			// ie7 must use absolute positioning in quirks mode and to
			// account
		...
		function remove(el, opts) {
			...
			var els;
			if (full) // crazy selector to handle odd field errors in
				// ie6/7
		...
		function reset(els, data, opts, el) {
			...
			// fix issue in Safari 6 where block artifacts remain until
			// reflow
		...
		function bind(b, el, opts) {
			...
			// don't bind events when overlay is not in use or if bindEvents
			// is
			// false
